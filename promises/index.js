...
function showTagInfo(tag) {
  getTagItemsAsync(tag.id, (err, res) => {
    if (err) {
...
function getCurrencyRate(code) {
  return new Promise((resolve, reject) => {
    getCurrencyRateAsync(code, (err, res) => {
      if (err) {
        reject(err);
      } else {
        resolve(res);
...
function convertCurrency(amount, fromCode, toCode) {
  return Promise.all([getCurrencyRate(fromCode), getCurrencyRate(toCode)])
    .then(res => (amount * res[0] / res[1]).toFixed(2))
    .catch(err => {
...
function getTagItemsCountAsync(title, cb) {
  getTagAsync(title, (err, tag) => {
    if (err) {
      cb(`[Error: ${err}]`);
    } else {
      getTagItemsAsync(tag.id, (err, items) => {
        if (err) {
          cb(`[Error: ${err}]`);
        } else {
