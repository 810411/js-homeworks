...
class AllDayExceptHolidaysPaymentTerminal extends PaymentTerminal {
  ...
  checkActive() {
    const month = this.calendar.now.getMonth();
    const date = this.calendar.now.getDate();
    return !(this.holidays.find(item => item.month === month && item.date === date));
  }
}
